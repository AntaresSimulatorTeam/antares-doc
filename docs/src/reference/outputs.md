...
In economy and adequacy simulations, the optimization ignores the primary and strategic reserves. Hhowever, it may include spinning and day-ahead reserves, depending on the settings made in the **Configuration** > **Optimization** tab.

**In adequacy simulations,** all dispatchable thermal units are given the `must-run` status (hence, they will generate at maximum power, regardless of the demand). As a consequence the only variables that are actually meaningful are the adequacy indicators (unsupplied energy, LOLD, LOLP), that may depend on assumptions made regarding the economic values of Unsupplied and spilled energies, and on hurdle costs on interconnections.
