...
Cluster modelling brings a significant evolution in the Antares data model that has an impact on the Input/Output data formats and in the User Interface (see following chapters) and this can lead to a significant increase in the memory space occupied by a study.
...
However, this feature does not affect the resolution behavior. Therefore, thes 2 sets are equivalent :
