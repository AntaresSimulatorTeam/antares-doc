...
Antares Solver allows conducting hybrid (solver x [modeler](./gems-overview.md)) studies.  
This enables using [antares-solver](../programmatic/index.md) studies, in which the simulated system is enriched with components 
that are described in the `antares-modeler` format.

## Input structure
To define a hybrid study, define a solver study, and simply add the [modeler files and directories](./gems-inputs.md) 
to the input directory.    
The parameter.yml file from modeler studies is not needed (if it exists, it will be ignored). The solver parameters
are used, since hybrid studies are conducted using antares-solver.  
...
the [modeler outputs page](./simulation-table.md), in the same output folder as the legacy outputs.  
...
using the new [modelling language](https://gems-energy.readthedocs.io/en/latest/), and inserting these system components into existing studies, 
by connecting them to "areas". 

Such connections allow : 

- adding generation or consumption to an existing area, thus modifying its balance constraint.
...
[system file](./gems-inputs.md#system-file). 
...
- **component**: the IDs of the component to connect to the area, as defined in the [components section](./gems-inputs.md#components)
...
This is done in the [model library](./gems-inputs.md#model-libraries), in the [port type definition](./gems-inputs.md#port-types).  
...
The nature of this contribution depends on the field : 
  ...
  - **injection-to-balance**: the linear expression is injected in the balance constraint of the area.
	This is done with respect to a convention (see next [Optimization model](./optimization-problem.md)).
...
Check that your model respects the internal optimization model's injection [convention](./optimization-problem.md).