...
- _**root directory with any name**_
    ...
    - **parameters.yml**: the [parameters](./gems-parameters.md) file

Note that Antares will automatically create an **output** directory to write the modeler [outputs](simulation-table.md).
...
The **port-types** collection lists the possible types of ports inside the library,
...
- **area-connection** _(optional)_: used only for hybrid (solver x modeler) studies, ignored
  ...
  studies, [see the relevant documentation](./hybrid-studies.md#selecting-the-area-connection-port-fields).
...
- **extra-outputs** _(optional)_: a collection of outputs relevant to the model, that can be computed after optimization
  ...
  The values of these extra outputs will appear in the [output files](./simulation-table.md) along with variable and port values.
...
[An expression](https://gems-energy.readthedocs.io/en/latest/user-guide/syntax/) 
is a human-readable equation that allows a large flexibility in defining objects of optimization.

## System file

The system file describes the energy system that is to be simulated, by listing the components
...
by `antares-modeler`.
For more information on hybrid
studies, [see the relevant documentation](./hybrid-studies.md#connecting-modeler-components-to-legacy-areas).
...
numeric values), but does not validate that the data covers the full [time horizon](gems-parameters.md#horizon) of the
